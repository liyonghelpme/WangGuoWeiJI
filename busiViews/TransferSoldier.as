class TransferSoldier extends MyNode
{
    ...
    function initData()
    {
        ...
        {
            var sData = getData(SOLDIER, its[i][1]["id"]);
            var ret = checkTransfer(its[i][1]["level"], sData);
            if(ret == 1 && its[i][1]["dead"] == 0)//只有没有阵亡的士兵
                data.append(its[i]);
        }
    }
    var selled = 0;
    function onTransfer()
    {
        if(soldier != null)
        {
            var cost = getTransferCost(soldier[0]);
            ...
            global.httpController.addRequest("soldierC/doTransfer", dict([["uid", global.user.uid], ["sid", soldier[0]], ["crystal", cost["crystal"]]]), null, null);
    ...
    function TransferSoldier(d)
    {
        ...
        var but0 = new NewButton("blueButton.png", [150, 44], getStr("", null), null, 23, FONT_NORMAL, [100, 100, 100], onTransfer, null);
        but0.bg.pos(130, 432);
        addChild(but0);
        but0.bg.addsprite("crystal.png").anchor(0, 0).pos(21, 8).size(27, 27).color(100, 100, 100, 100);
        transferLabel = but0.bg.addlabel(getStr("transferBut", null), "fonts/heiti.ttf", 20).anchor(0, 50).pos(52, 21).color(100, 100, 100);
    ...
    */
    ...
    function setSoldier(sol)
    {
        ...
        if(soldier == null)
            transferLabel.text(getStr("transferBut", ["[NUM]", str(0)]));
        else
        {
            var cost = getTransferCost(soldier[0]);
            var it = cost.values();
            transferLabel.text(getStr("transferBut", ["[NUM]", str(it[0])]));
        }
