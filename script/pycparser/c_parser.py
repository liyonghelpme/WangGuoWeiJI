...
class CParser(PLYParser):    
    ...
    def _add_declaration_specifier(self, declspec, newspec, kind):
        #print "add_declaration_specifier", declspec, newspec, kind
    ...
    def p_new_object(self, p):
        """new_object : NEW identifier LPAREN argument_expression_list RPAREN
        """
        p[0] = c_ast.NewObject(p[2], p[4])
    ...
    def p_array(self, p):
        ...
        """
        p[0] = c_ast.Array(None, coord=self._coord(p.lineno(1)))
    def p_array_1(self, p):
        """ array : LBRACKET argument_expression_list  RBRACKET
        """
        p[0] = c_ast.Array(p[2], coord=self._coord(p.lineno(1)))
