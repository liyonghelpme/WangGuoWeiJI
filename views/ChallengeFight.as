    ...
    function ChallengeFight(win, star, fightReward, score, m, levelUpSol)//reward,
    {
        ...
        if(win == 1)
        {
            ...
            if(fightReward.get("crystal", 0) > 0)
                bg.addlabel(getStr("rewardCry", ["[NUM]", str(fightReward.get("crystal"))]), null, 18).pos(83, 256).color(0, 0, 0);
            else if(fightReward.get("gold", 0) > 0)
                bg.addlabel(getStr("rewardGold", ["[NUM]", str(fightReward.get("gold"))]), null, 18).pos(83, 256).color(0, 0, 0);
        }
        //失败显示 丢失的得分
        else
        {
