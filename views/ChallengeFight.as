class ChallengeFight extends MyNode
{
    ...
    */
    ...
    function ChallengeFight(win, star, fightReward, score, m, levelUpSol)//reward,
    {
        map = m;
...
bg.addlabel(levelupStr, "fonts/heiti.ttf", 18).pos(83, 212).color(0, 0, 0);
            if(fightReward.get("crystal", 0) > 0)
bg.addlabel(getStr("rewardCry", ["[NUM]", str(fightReward.get("crystal"))]), "fonts/heiti.ttf", 18).pos(83, 256).color(0, 0, 0);
            else if(fightReward.get("gold", 0) > 0)
bg.addlabel(getStr("rewardGold", ["[NUM]", str(fightReward.get("gold"))]), "fonts/heiti.ttf", 18).pos(83, 256).color(0, 0, 0);
        }
        //失败显示 丢失的得分
        else
        {
