class GiftView extends MyNode
{
    ...
    function GiftView(p, s)
    {
        ...
        but0.addlabel(getStr("recAll", null), null, 25).color(0, 0, 0).anchor(50, 50).pos(74, 26);
    ...
    */
    ...
    function updateTab()
    {
        ...
        for(var i = rg[0]; i < rg[1]; i++)
        {
            ...
            panel.addlabel(getStr("friSendGift", ["[NAME]", data[i][0], "[NUM]", str(data[i][2]), "[KIND]", getStr(data[i][1], null)]), null, 25).pos(42, 20).anchor(0, 0).color(78, 78, 78);
...
class MoreView extends MyNode
{
    ...
    */
    ...
    function updateTab()
    {
        ...
        {
            var panel = flowNode.addsprite("dialogMakeDrugBanner.png").pos(0, OFFY*i).size(PANEL_WIDTH, PANEL_HEIGHT);
            panel.addlabel(data[i][0], null, 25).pos(122, 20).anchor(0, 0).color(78, 78, 78);
            var but0 = panel.addsprite("greenButton.png").pos(636, 30).size(118, 45).setevent(EVENT_TOUCH, onDownload, i);
            but0.addlabel(getStr("download", null), null, 22).color(0, 0, 0).pos(59, 22).anchor(50, 50);
        }
    }
