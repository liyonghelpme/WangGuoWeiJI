...
class UpgradeDialog extends MyNode
{
    ...
    function UpgradeDialog(s, e)
    {
        ...
        init();
bg.addlabel(getStr("upgradeEquip", null), "fonts/heiti.ttf", 30).anchor(50, 50).pos(264, 26).color(33, 34, 40);

        bg.addsprite("roleNameClose.png").pos(499, 9).setevent(EVENT_TOUCH, closeDialog);
words = bg.addlabel("", "fonts/heiti.ttf", 18).pos(264, 122).anchor(50, 50).color(0, 0, 0);
bg.addlabel(getStr("brokenEquip", null), "fonts/heiti.ttf", 18).pos(264, 303).anchor(50, 50).color(100, 0, 0);

        var scroll = bg.addsprite("dialogScroll.png").anchor(50, 50).pos(264, 81);
scroll.addlabel(getStr("useStoneEquip", null), "fonts/heiti.ttf", 22).pos(252, 32).anchor(50, 50).color(27, 15, 4);
...
pan.addlabel(getStr("sucPos", ["[POS]", str(pos0)]), "fonts/heiti.ttf", 18).pos(57, 20).anchor(50, 50).color(31, 17, 5);

            var num = global.user.getGoodsNum(TREASURE_STONE, i);
pan.addlabel(getStr("treaNum", ["[NUM]", str(num)]), "fonts/heiti.ttf", 18).pos(57, 144).anchor(50, 50).color(0, 0, 0);
        }



        var but0 = bg.addsprite("roleNameBut0.png").pos(147, 347).anchor(50, 50).size(214, 65).setevent(EVENT_TOUCH, onBuy);
but0.addlabel(getStr("buyIt", null), "fonts/heiti.ttf", 35).pos(107, 32).anchor(50, 50).color(100, 100, 100);
        but1 = bg.addsprite("roleNameBut0.png").pos(377, 347).anchor(50, 50).size(214, 65).setevent(EVENT_TOUCH, onUpgrade);
but1.addlabel(getStr("upgrade", null), "fonts/heiti.ttf", 35).pos(107, 32).anchor(50, 50).color(100, 100, 100);
