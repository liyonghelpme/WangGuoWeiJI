...
class UpgradeDialog extends MyNode
{
    ...
    function UpgradeDialog(s, e)
    {
        ...
        bg.addsprite("dialogScroll.png").anchor(50, 50).pos(264, 81);
        ...
        if(rcode != 0)
        {
            ...
            if(con.get("id") != 0)
            {
                if(con.get("suc") == 1)
                {
                    ...
                    scene.addChild(new UpgradeBanner(getStr("sucUpgrade", null), [0, 100, 0]));
                }
                else
                {
                    if(con.get("breakEquip") == 0)
                    {
                        scene.addChild(new UpgradeBanner(getStr("failUpgrade", null), [100, 0, 0]));
                    }
                    else
                    {
                        global.user.breakEquip(eid);
                        scene.addChild(new UpgradeBanner(getStr("failEquip", null), [100, 0, 0]));
                    }
                }
            }
        }
